...
uint64_t funVnodeResearch(uint64_t kfd, char* to, char* from) {
    int file_index = open(to, O_RDONLY);
    if (file_index == -1) return -1;
    ...
    uint32_t to_iocount = kread32(kfd, to_vnode + off_vnode_v_iocount);
    ...
    uint32_t from_usecount = kread32(kfd, from_vnode + off_vnode_v_usecount);
    printf("[i] %s from_vnode->v_usecount: %d\n", from, from_usecount);
    uint32_t from_iocount = kread32(kfd, from_vnode + off_vnode_v_iocount);
    printf("[i] %s from_vnode->v_iocount: %d\n", from, from_iocount);
    uint32_t from_v_kusecount = kread32(kfd, from_vnode + off_vnode_v_kusecount);
    printf("[i] %s from_vnode->v_kusecount: %d\n", from, from_v_kusecount);
...
//    kwrite32(kfd, from_vnode + off_vnode_v_flag, kread32(kfd, from_vnode + off_vnode_v_flag) & VFMLINKTARGET);
//
